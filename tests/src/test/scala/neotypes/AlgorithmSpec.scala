...
sealed trait BaseAlgorithmSpec[F[_]] extends CleaningIntegrationSpec[F] { self: DriverProvider[F] =>
  ...
  it should "execute the article rank centrality algorithm" in executeAsFuture { d =>
    for {
      ...
      result <- """CALL gds.articleRank.stream('papersGraph', {
                   ...
                   LIMIT 5
                """.query(productDerive[ScoredPaper]).list(d)
      _ <- "CALL gds.graph.drop('papersGraph', false)".execute.void(d)
    } yield {
    ...
    for {
      ...
      result <- """CALL gds.triangleCount.mutate('peopleGraph', {
                   ...
                   ORDER BY
                     ...
                     person ASC
                   LIMIT 5
                """.query(productDerive[PersonTriangleCount]).list(d)
      _ <- "CALL gds.graph.drop('peopleGraph', false)".execute.void(d)
    } yield {
      result shouldBe List(
  ...
  it should "execute the shortest path algorithm" in executeAsFuture { d =>
    for {
      ...
      result <- """MATCH (start: Location { name:'A' }), (target: Location { name:'F' })
                ...
                """.query(productDerive[ShortestPath]).single(d)
      _ <- "CALL gds.graph.drop('locationsGraph', false)".execute.void(d)
    } yield {
