...
import neotypes.{CleaningIntegrationWordSpec, FutureTestkit}
...
final class RefinedSpec extends CleaningIntegrationWordSpec[Future](FutureTestkit) {
  import RefinedSpec.{Level, User}
  "refined" should {
    "insert and retrieve one refined value" in executeAsFuture { s =>
      val L1: Level = 1

      for {
        _ <- c"CREATE (level: Level { value: ${L1} })".query[Unit].execute(s)
        level <- "MATCH (level: Level) RETURN level.value".query[Level].single(s)
      } yield level shouldBe L1
    }

    "insert and retrieve multiple refined values" in executeAsFuture { s =>
      val L1: Level = 1
      val L2: Level = 2
      ...
      } yield levels shouldBe List(L1, L2)
    }

    "insert and retrieve wrapped refined values" in executeAsFuture { s =>
      val L1: Level = 1
      val L2: Level = 2
      ...
      } yield levels shouldBe levels
    }

    "retrieve refined values inside a case class" in executeAsFuture { s =>
      for {
      ...
      } yield user shouldBe User(name = "Balmung", level = 99)
    }

    "fail if a single value does not satisfy the refinement condition" in executeAsFuture { s =>
    ...
    "fail if at least one of multiple values does not satisfy the refinement condition" in executeAsFuture { s =>
      recoverToSucceededIf[IncoercibleException] {
        for {
    ...
    "fail if at least one wrapped value does not satisfy the refinement condition" in executeAsFuture { s =>
      recoverToSucceededIf[IncoercibleException] {
        for {
    ...
    "fail if at least one value inside a case class does not satisfy the refinement condition" in executeAsFuture { s =>
      recoverToSucceededIf[IncoercibleException] {
        for {
