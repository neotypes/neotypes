...
import _root_.cats.data.{Chain, Const, NonEmptyChain, NonEmptyList, NonEmptyMap, NonEmptySet, NonEmptyVector}
...
object mappers extends LowPriorityMappers {
  ...
  implicit final def nonEmptyChainParameterMapper[A](implicit
    mapper: ParameterMapper[A]
  ): ParameterMapper[NonEmptyChain[A]] =
  ...
  implicit final def nonEmptyListParameterMapper[A](implicit
    mapper: ParameterMapper[List[A]]
  ): ParameterMapper[NonEmptyList[A]] =
  ...
  implicit final def nonEmptyVectorParameterMapper[A](implicit
    mapper: ParameterMapper[Vector[A]]
  ): ParameterMapper[NonEmptyVector[A]] =
    mapper.contramap(_.toVector)

  // NonEmptySet.
  implicit final def nonEmptySet[A](implicit
  ...
  implicit final def nonEmptySetParameterMapper[A](implicit
    mapper: ParameterMapper[SortedSet[A]]
  ): ParameterMapper[NonEmptySet[A]] =
    mapper.contramap(_.toSortedSet)

  // NonEmptyMap.
  implicit final def nonEmptyNeoMap[K, V](implicit
  ...
  implicit final def nonEmptyNeoMapParameterMapper[K, V](implicit
    mapper: ParameterMapper[SortedMap[K, V]]
  ): ParameterMapper[NonEmptyMap[K, V]] =
    mapper.contramap(_.toSortedMap)
}

private[data] sealed abstract class LowPriorityMappers {
  implicit final def nonEmptyMap[K, V](implicit
  ...
  implicit final def nonEmptyMapParameterMapper[K, V](implicit
    mapper: ParameterMapper[(K, V)]
  ): ParameterMapper[NonEmptyMap[K, V]] =
