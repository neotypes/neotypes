...
trait ResultMapper[A] { self =>
  ...
  /** Combines the result of another mapper with this one. In case both fail, the errors will be merged into a
    * [[model.exceptions.ChainException]].
    */
  ...
  /** Chains a mapper as a fallback when this one fails. In case both fail, the errors will be merged into a
    * [[model.exceptions.ChainException]].
    */
    ...
    ResultMapper.instance { value =>
      self.decode(value).left.flatMap { thisEx =>
        other.decode(value).left.map { otherEx =>
          ChainException.from(exceptions = thisEx, otherEx)
        }
      }
    }
...
object ResultMapper extends ResultMappersLowPriority0 with BoilerplateResultMappers {
  ...
  def fromMatch[A](
    ...
    val fail: PartialFunction[NeoType, Either[ResultMapperException, A]] = { case value =>
  ...
  def fromNumeric[A](
    ...
    fromMatch { case value: Value.NumberValue =>
      f(value)
    }
  ...
  implicit final val bytes: ResultMapper[ArraySeq[Byte]] =
    fromMatch { case Value.Bytes(value) =>
      value
    }
  ...
  /** Creates a [[ResultMapper]] that will try to decode the first / single element list of values, using the provided
    * mapper.
    *
    * @note
  ...
  /** Creates a new [[ResultMapper]] from a base one, which will recover from `null` values into a `None`.
    */
  implicit final def option[A](implicit
    mapper: ResultMapper[A]
  ): ResultMapper[Option[A]] =
    fromMatch {
  ...
  implicit final def either[A, B](implicit
    a: ResultMapper[A],
    b: ResultMapper[B]
  ...
  /** Creates a [[ResultMapper]] that will try to decode the given element of a list of values, using the provided
    * mapper.
    *
    * @note
  ...
  /** Creates a [[ResultMapper]] for a coproduct. Based on a given [[CoproductDiscriminatorStrategy]], and the tagged
    * [[ResultMapper]]s of each case.
    */
  def coproduct[S, A](
    ...
    case CoproductDiscriminatorStrategy.Field(fieldName, fieldResultMapper) =>
      field(key = fieldName, mapper = fieldResultMapper).flatMap { label =>
        options
          .collectFirst { case (`label`, mapper) =>
            mapper.widen[A]
          }
          .getOrElse(
...
private[mappers] sealed abstract class ResultMappersLowPriority0 extends ResultMappersLowPriority1 {
  ...
  /** Creates a [[ResultMapper]] that will try to decode a [[neotypes.model.types.NeoObject]] into a some form of
    ...
    */
  implicit final def collectAsNeoMap[K, V, M[_, _]](implicit
    mapFactory: Factory[(K, V), M[K, V]],
    keyMapper: KeyMapper[K],
    valueMapper: ResultMapper[V]
  ): ResultMapper[M[K, V]] =
    neoObject.emap { obj =>
      traverseAs(mapFactory)(obj.properties) { case (key, value) =>
        keyMapper.decodeKey(key) and valueMapper.decode(value)
      }
    }
}

private[mappers] sealed abstract class ResultMappersLowPriority1 extends ResultMappersLowPriority2 {
  self: ResultMapper.type =>

  /** Creates a [[ResultMapper]] that will collect all values into the homogeneous provided collection, using the
    * provided mapper to decode each element.
    */
  implicit final def collectAs[A, C](implicit
    factory: Factory[A, C],
    mapper: ResultMapper[A]
  ): ResultMapper[C] =
  ...
  implicit final def collectAsIterable[A, C[_]](implicit
    factory: Factory[A, C[A]],
    mapper: ResultMapper[A]
  ): ResultMapper[C[A]] =
    collectAs(factory, mapper)

  /** Creates a [[ResultMapper]] that will collect all values into an homogeneous map-like collection, using the
    * provided mapper to decode each key-value element. Overload of [[collectAs]] to help implicit inference, don't call
    * explicitly.
    */
  implicit final def collectAsMap[K, V, M[_, _]](implicit
    mapFactory: Factory[(K, V), M[K, V]],
...
private[mappers] sealed abstract class ResultMappersLowPriority2 extends ResultMappersLowPriority3 {
  self: ResultMapper.type =>
