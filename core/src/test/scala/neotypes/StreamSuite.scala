...
abstract class StreamTestkit[F[_], S[_]](val effectTestkit: EffectTestkit[F])(implicit ctS: ClassTag[S[_]]) {
...
abstract class BaseStreamSpec[F[_], S[_]] (protected val streamTestkit: StreamTestkit[F, S]) extends BaseEffectSpec(streamTestkit.effectTestkit) with AsyncTestSuite {
...
abstract class StreamSessionProvider[F[_], S[_]] (testkit: StreamTestkit[F, S]) extends BaseStreamSpec[F, S](testkit) with SessionProvider[F] { self: BaseIntegrationSpec[F] =>
  override protected final val sessionType: String = "RxSession"

  override protected final lazy val neotypesSession: StreamingSession[F, S] =
    Session[F, S](F, S, this.driver.rxSession())(fToT(F.makeLock))

  protected final def executeAsFutureList[T](work: StreamingSession[F, S] => S[T]): Future[List[T]] =
...
abstract class StreamSuite[F[_], S[_]](testkit: StreamTestkit[F, S]) extends Suites(
  new StreamSessionProvider(testkit) with StreamIntegrationSpec[F, S],
