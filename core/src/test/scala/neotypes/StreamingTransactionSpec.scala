...
) extends StreamingDriverProviderWordSpec[S, F](testkit) with CleaningIntegrationSpec[F] with Matchers {
  s"StreamingTransaction[${streamName}, ${effectName}]" should {
    "explicitly commit a transaction" in {
    ...
    "explicitly rollback a transaction" in {
      executeAsFutureList { d =>
        d.streamingTransaction.evalMap { tx =>
          for {
            _ <- "CREATE (p: PERSON { name: \"Luis\" })".query[Unit].execute(tx)
