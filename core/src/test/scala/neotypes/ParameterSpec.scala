...
final class ParameterSpec[F[_]](testkit: EffectTestkit[F]) extends AsyncDriverProvider[F](testkit) with CleaningIntegrationSpec[F] with Matchers {
  s"Inserting parameters for ${effectName}" should {
    "convert parameters" in executeAsFuture { d =>
  ...
  "convert map-like parameters into a node" in executeAsFuture { d =>
